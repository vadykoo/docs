...
Канали присутності базуються на безпеці приватних каналів, одночасно виявляючи додаткову функцію обізнаності про те, хто підписаний на канал. Це спрощує створення потужних спільних функцій додатків, таких як сповіщення користувачів, коли інший користувач переглядає ту саму сторінку.
...
З’єднавши Broadcast події з[повідомлення](/docs/{{version}}/notifications), ваш додаток JavaScript може отримувати нові сповіщення в міру їх появи, не потребуючи оновлення сторінки. По-перше, обов’язково прочитайте документацію щодо використання[канал сповіщення про Broadcast](/docs/{{version}}/notifications#broadcast-notifications).

Після того, як ви налаштували сповіщення для використання Broadcast каналу, ви можете слухати Broadcastні події за допомогою Echo's`notification`метод. Пам’ятайте, назва каналу повинна відповідати назві класу сутності, яка отримує сповіщення:
...
У цьому прикладі всі сповіщення надіслані на`App\Models\User`екземпляри через`broadcast`канал отримає зворотний дзвінок. Зворотний виклик авторизації каналу для`App.Models.User.{id}`канал включений за замовчуванням`BroadcastServiceProvider`що поставляється з рамкою Laravel.
