...
Cashierр включає в себе`paddle-button`Компонент леза. Ми можемо передати URL-адресу посилання на оплату цьому компоненту як "підставку". Після натискання цієї кнопки відображатиметься віджет оформлення замовлення Paddle:
...
Щоб полегшити вам початок роботи з вбудованою Cashier, Cashier включає в себе`paddle-checkout`Компонент леза. Для початку вам слід[сформувати посилання на оплату](#pay-links)і передайте посилання на оплату компоненту`override`атрибут:
